    ...
    public partial class Edicao : System.Web.UI.Page
    {
                    ...
                    using (SqlCommand cmd = new SqlCommand("SELECT TipoMercadoria , Nome, Quantidade, Preco, TipoNegocio FROM tbProduto WHERE Id = @id", conn))
                    {
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            if (reader.Read() == true)
                            {
                                ...
                                txtPreco.Text = reader.GetDouble(3).ToString("0.00");
        ...
        protected void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = Sql.OpenConnection())
            {
                ...
                using (SqlCommand cmd = new SqlCommand("UPDATE tbProduto SET TipoMercadoria = @mercadoria, Nome = @nome, Quantidade = @quantidade, Preco = @preco, TipoNegocio = @negocio WHERE Id = @id", conn))
                {
